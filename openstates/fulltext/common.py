...
def extractor_for_element_by_xpath(
    bill_text_element_selector: str, preserve_element_boundaries: bool = False
) -> ExtractorFunc:
    """OPEN-212: pass `preserve_element_boundaries=True` to serialize with `xml_tree_to_lines()` rather than
    lxml's separator-free `text_content()`. See `text_from_element_xpath` for why the default
    is unchanged."""

    def _my_extractor(data: bytes, metadata: Metadata) -> str:
        text_inside_matching_tag = text_from_element_xpath(
            data, bill_text_element_selector,
            preserve_element_boundaries=preserve_element_boundaries
        )
        return clean(text_inside_matching_tag)
...
# OPEN-210's XML/HTML serializer lives in utils.py so that module can use it too
# (OPEN-212 needs it for Washington HTML, and utils.py cannot import from common.py --
# the dependency runs the other way). Re-exported here because ut.py/us.py import it
# from this module.
from .utils import xml_tree_to_lines  # noqa: E402,F401  (re-export)
