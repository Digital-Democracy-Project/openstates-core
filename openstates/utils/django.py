...
def _resolve_database_url() -> str:
    ...
    OPEN-260: RESOLVE_RDS_LIVE=true opts into resolving the current RDS credential from Secrets
    ...
    pm-review: matches this codebase's own `os.getenv(X, "false").lower() == "true"` convention
    (ddp-sync's config.py) rather than a bare truthiness check -- RESOLVE_RDS_LIVE=0 or =false
    must NOT enable this, which a plain `if os.environ.get(...)` would have gotten wrong (any
    non-empty string, including "0" and "false", is truthy in Python).
    """
